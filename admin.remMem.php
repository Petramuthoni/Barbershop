  ...
  <header>
    <div class="main-nav-bar" id="header-bar">
      <ul class="main-nav">
        ...
        <li><a class="active" href="admin.remMem.php"> Remove Customers </a></li>
...
<center><h1>Remove Customer</h1><hr>
<form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">  
Enter CID:<center><input type="number" name="did"></center>
			<button type="submit" name="Submit1">Delete by Customer ID</button>
			<br>---------OR---------<br>
Select Name:<br><?php
				require_once('admin.dbconfig.php');
				$emp_result = $conn->query('select * from customer order by CustomersID ASC');
				?>
				<center>
				<select name="Name">
				...
				?>
				<option value="<?php echo $row["CustomersID"]; ?>"><?php echo "$row[CustomersID] ".$row["Name"]; ?></option>
				<?php
					}
					}
				?>
...
{
	$did=$_POST['did'];
	$sql = "DELETE FROM customer WHERE CustomersID= $did ";
	if (mysqli_query($conn, $sql))
		{
		echo "Record deleted successfully from Customer table.Refreshing....";
		header( "Refresh:3; url=admin.remMem.php");
		}
	else
		{
